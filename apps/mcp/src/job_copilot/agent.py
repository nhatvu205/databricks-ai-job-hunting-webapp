...
class JobAgent:
    ...
    async def respond(self, actor: Actor, message: str) -> str:
        ...
        # Databricks-managed foundation models are Unity AI Gateway model services,
        # not workspace Model Serving endpoints.
        client = OpenAI(base_url=f"{host.rstrip('/')}/ai-gateway/mlflow/v1", api_key=token)
        ...
        for _ in range(6):
            completion = client.chat.completions.create(
                model=self.settings.model_service, messages=messages, tools=tools, temperature=0.2
            )
